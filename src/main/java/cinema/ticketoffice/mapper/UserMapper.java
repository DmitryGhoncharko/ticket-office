...
@Component
public class UserMapper extends GenericMapper<User, UserDTO> implements ConverterForSpecificFields<User, UserDTO> {
    ...
    public void setupMapper() {
        modelMapper.createTypeMap(User.class, UserDTO.class).addMappings(m -> m.skip(UserDTO::setOrdersIds)).setPostConverter(toDtoConverter());

        modelMapper.createTypeMap(UserDTO.class, User.class).addMappings(m -> m.skip(User::setOrders)).setPostConverter(toEntityConverter());
    }
    ...
    public void mapSpecificFields(User source, UserDTO destination) {
        ...
        if (orders != null) {
            ordersIds = orders.stream().map(GenericModel::getId).collect(Collectors.toSet());
        }
