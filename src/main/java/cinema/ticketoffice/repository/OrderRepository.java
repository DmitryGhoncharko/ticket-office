...
public interface OrderRepository extends GenericRepository<Order> {
    ...
    @Query("SELECT new       cinema.ticketoffice.dto.OrderInfoDTO(o.id, o.createdWhen, f.title, o.cost, o.purchase) " + "FROM Film f JOIN f.filmSessions fs JOIN fs.orders o WHERE o.user.id = ?1 AND o.isDeleted = false ORDER BY o.createdWhen DESC")
    Page<OrderInfoDTO> getAllInfoByUserId(Long userId, PageRequest pageRequest);

    @Query(nativeQuery = true, value = "select sum(cost) from orders " + "where created_when between :startDate and :endDate " + "and is_deleted = false and purchase = true")
