...
public class FilmRestController extends GenericRestController<Film, FilmDTO> {
    ...
    @Operation(summary = "Добавить создателя к фильму", description = "Позволяет добавить создателя к фильму по указанным идентификаторам")
    @PutMapping(value = "/{filmId}/addFilmCreator/{filmCreatorId}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<FilmDTO> addFilmCreator(@PathVariable(value = "filmId") @Parameter(description = "Идентификатор фильма") Long filmId, @PathVariable(value = "filmCreatorId") @Parameter(description = "Идентификатор создателя") Long filmCreatorId) {
    ...
    @Operation(summary = "Получить все актуальные фильмы", description = "Позволяет получить список всех фильмов с киносеансами в будущем")
    ...
    public ResponseEntity<List<FilmDTO>> getAllAvailable() {
        ...
        return availableFilms.isEmpty() ? ResponseEntity.notFound().build() : ResponseEntity.ok(availableFilms);
    }
}
