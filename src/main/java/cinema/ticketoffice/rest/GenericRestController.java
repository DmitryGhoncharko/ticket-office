...
import cinema.ticketoffice.service.GenericService;
...
public abstract class GenericRestController<E extends GenericModel, D extends GenericDTO> {
    ...
    public ResponseEntity<List<D>> getAll() {
        ...
        return resultingList.isEmpty() ? ResponseEntity.status(HttpStatus.NO_CONTENT).build() : ResponseEntity.status(HttpStatus.OK).body(resultingList);
    }
    ...
    public ResponseEntity<D> getById(@PathVariable("id") @Parameter(description = "Идентификатор записи") Long id) {
        ...
        return foundDto != null ? ResponseEntity.ok(foundDto) : ResponseEntity.notFound().build();
    }
    ...
    public ResponseEntity<D> create(@RequestBody D newDto) {
        ...
        return createdDto != null ? ResponseEntity.status(HttpStatus.CREATED).body(createdDto) : ResponseEntity.status(HttpStatus.CONFLICT).build();
    }
    ...
    public ResponseEntity<D> update(@RequestBody D toUpdateDto) {
        ...
        return updatedDto != null ? ResponseEntity.ok(updatedDto) : ResponseEntity.status(HttpStatus.NOT_ACCEPTABLE).build();
    }

    @Operation(summary = "Удалить запись по ID", description = "Позволяет удалить запись по заданному ID")
    @DeleteMapping(value = "/delete/{id}")
    public ResponseEntity<D> deleteById(@PathVariable("id") @Parameter(required = true, description = "Идентификатор записи") Long id) {
