...
public class OrderRestController extends GenericRestController<Order, OrderDTO> {
    ...
    @Operation(summary = "Получить заказы по ID пользователя", description = "Позволяет получить список заказов по заданному ID пользователя")
    @GetMapping(value = "/user/{userId}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<List<OrderDTO>> getByUserId(@PathVariable("userId") @Parameter(description = "Идентификатор пользователя") Long userId) {
        List<Order> orders = userService.getOrders(userId);
        return orders != null ? ResponseEntity.ok(orderMapper.toDTOs(orders)) : ResponseEntity.notFound().build();
    }
